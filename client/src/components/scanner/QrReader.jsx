...
export default function QrReader() {
  ...
  return (
    <div className="w-full px-2 sm:px-0 flex justify-center">
      <div className="w-full max-w-md bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
        ...
        {loading && (
          <div className="p-8 sm:p-12 text-center bg-white">
            <RefreshCw
              className="animate-spin mx-auto text-indigo-600 mb-3 sm:mb-4"
              size={40}
            />
        ...
        {result && (
          <div
            className={`p-6 sm:p-8 text-center animate-in zoom-in-95 duration-200 ${result.type === "success" ? "bg-green-50" : "bg-red-50"}`}
          >
            {result.type === "success" ? (
              <CheckCircle2
                size={60}
                className="sm:size-80 mx-auto text-green-500 mb-3 sm:mb-4"
              />
            ) : (
              <XCircle
                size={60}
                className="sm:size-80 mx-auto text-red-500 mb-3 sm:mb-4"
              />
            )}

            <h3
              className={`text-lg sm:text-2xl font-black mb-4 sm:mb-6 ${result.type === "success" ? "text-green-800" : "text-red-800"}`}
            >
              {result.message}
            </h3>

            {result.participant && (
              <div className="text-left bg-white/80 backdrop-blur-sm p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-white/50 space-y-3 sm:space-y-4 mb-6 sm:mb-8 shadow-sm text-sm">
                <div>
            ...
            <div className="space-y-2 sm:space-y-3">
              <button
                ...
                className="w-full py-3 sm:py-4 bg-indigo-600 text-white rounded-xl sm:rounded-2xl font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-[0.98] transition-all text-sm sm:text-base"
              >
                Scan Next Ticket
              </button>
              ...
              <Link
                to="/"
                className="w-full py-3 sm:py-4 bg-white text-slate-600 rounded-xl sm:rounded-2xl font-bold border border-slate-200 hover:bg-slate-50 flex items-center justify-center gap-2 transition-all text-sm sm:text-base"
              >
